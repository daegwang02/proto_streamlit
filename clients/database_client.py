...
class DatabaseClient:
    def __init__(self, data_url: str, transaction_fee_buy: float = 0.0005, transaction_fee_sell: float = 0.0015):
        ...
        self._factor_id_counter = 0
        # 💡 'ast' 컬럼을 추가
        self.factors = pd.DataFrame(columns=['id', 'formula', 'description', 'status', 'ast', 'complexity_sl', 'originality_score', 'alignment_score'])
    ...
    def save_factor(self, data: Dict[str, Any]) -> int:
        data_to_save = data.copy()
        if 'ast' in data_to_save:
            # 💡 AST 객체를 pickle로 직렬화하여 저장
            data_to_save['ast'] = pickle.dumps(data_to_save['ast'])
    ...
    def get_new_factors(self) -> List[Dict[str, Any]]:
        """평가되지 않은 새로운 팩터들을 가져옵니다."""
        new_factors_df = self.factors[self.factors['status'] == 'new'].copy()
        if new_factors_df.empty:
            return []
        # 💡 pickle로 저장된 AST를 역직렬화
        new_factors_df['ast'] = new_factors_df['ast'].apply(lambda x: pickle.loads(x))
        # 💡 formula, ast, id를 모두 반환
        return new_factors_df[['id', 'formula', 'ast']].to_dict('records')
