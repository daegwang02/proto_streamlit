...
class LLMClient:
    ...
    def generate_factor_from_hypothesis(self, hypothesis: Dict[str, Any]) -> Dict[str, str]:
        ...
        4. **논리 연산자 사용법**: `and`와 `or`는 함수 형태로 사용하며, **반드시 비교 연산 결과(불리언 값)만을 인자로 받아야 합니다.**
            - **올바른 예시**: `and(close > ts_mean(close, 20), volume > ts_mean(volume, 20))`
            - **잘못된 예시**: `and(delta(close, 5), delta(volume, 3))` (delta는 불리언이 아닌 숫자 값을 반환하므로 오류 발생)
        5. **오류 방지**: 공식에 `and`나 `or` 같은 논리 연산자를 직접 사용하지 말고, `if` 함수 내에서 `&&`와 `||`로 변환하여 사용하거나, 별도의 함수로 분리하세요. 파서가 제대로 처리하지 못할 수 있습니다. (이전 프롬프트와 동일)
