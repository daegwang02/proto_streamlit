...
class LLMClient:
    ...
    def generate_factor_from_hypothesis(self, hypothesis: Dict[str, Any]) -> Dict[str, str]:
        ...
        [팩터 공식 생성 규칙]
        1. **단순성**: 팩터는 최대한 간결하고 단순하게 작성해야 합니다.
        ...
        4. **논리 및 비교 연산자 사용법**: `and`, `or`, `if` 함수는 논리적 조건을 처리하는 데 사용됩니다. **`>` 같은 비교 연산자는 항상 `and`, `or`, `if` 함수나 다른 연산자의 인자 내부에서 사용해야 합니다.** - **올바른 예시**: `and(close > ts_mean(close, 20), volume > ts_mean(volume, 20))`
           - **잘못된 예시**: `if(close > ts_mean(close, 20), ...)` (이 형식은 이미 파서에서 지원하지만, LLM이 혼란스러워할 수 있으므로 `and`나 `or`를 명시적으로 사용하는 것을 권장)
           - **절대 안되는 예시**: `close > ts_mean(close, 20) and volume > ts_mean(volume, 20)`
        5. **복잡한 조건**: `and`와 `or`는 반드시 함수 형태로 사용해야 합니다. 
           - 예시: `and(조건1, 조건2)` 또는 `or(조건1, 조건2)`
        6. **오류 방지**: 공식에 `and`나 `or` 같은 논리 연산자를 직접 사용하지 말고, 함수 형태로 사용하세요.
